...
class UserCollection {
  ...
  create(userData) {
    let newUser = new User(userData.surname, userData.name, userData.surname1, userData.destination, userData.date, userData.mass);
  ...
  //знаходиомо користувача по прізвищу
  getByUsername(surname) {
    return this.items.find(user => user.surname.toLowerCase() == surname.toLowerCase());
  }
  //знаходиомо користувача довжина якого коротша за вказану
  getShortUsernames(len) {
    return this.items.filter(user => user.surname.length <= len);
  }

  getByUsernameStart(searchString) {
    return this.items.filter(user => user.surname.toLowerCase().startsWith(searchString.toLowerCase()));
  }
